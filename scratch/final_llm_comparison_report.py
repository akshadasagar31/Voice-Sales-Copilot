...
from services.tts import DeepgramTTSService, get_shared_tts_client
...
def evaluate_model(model_name: str, query: str):
    ...
    return {
        ...
        "ttfa_ms": ttfa,
        "chunk0": chunk0,
        "answer": full_answer[:90],
    }

if __name__ == "__main__":
    q = "What is the minimum CIBIL score for personal loans?"
    print("\nRunning comprehensive benchmark...")
    for m in ["deepseek/deepseek-chat", "meta-llama/llama-3.3-70b-instruct"]:
        try:
            res = evaluate_model(m, q)
            print(f"\n=======================================================")
            print(f"MODEL: {res['model']}")
            print(f"TTFT (Time to First Token):     {res['ttft_ms']:6.1f} ms")
            print(f"Chunk #0 Ready:                 {res['chunk0_ms']:6.1f} ms ('{res['chunk0']}')")
            print(f"Deepgram Aura TTS Synthesis:    {res['tts_ms']:6.1f} ms")
            print(f"TOTAL TTFA (Time to 1st Audio): {res['ttfa_ms']:6.1f} ms")
